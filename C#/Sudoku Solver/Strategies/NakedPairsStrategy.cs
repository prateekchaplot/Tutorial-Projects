...
namespace Sudoku_Solver.Strategies
{
    class NakedPairsStrategy : ISudokuStrategy
    {
        ...
        private bool HasNakedPairInRow(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            ...
            {
                if (givenCol != col &&
                    IsNakedPair(sudokuBoard[givenRow, col], sudokuBoard[givenRow, givenCol]))
                    return true;
            }
        ...
        private bool HasNakedPairInCol(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            ...
            {
                if (givenRow != row &&
                    IsNakedPair(sudokuBoard[row, givenCol], sudokuBoard[givenRow, givenCol]))
                    return true;
            }
        ...
        {
            if (!HasNakedPairInGroup(sudokuBoard, givenRow, givenCol)) return;
        ...
        private bool HasNakedPairInGroup(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            var givenSudokuMap = _sudokuMapper.Find(givenRow, givenCol);
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    var elementSame = givenRow == row && givenCol == col;
                    var currentSudokuMap = _sudokuMapper.Find(row, col);
                    var elementInSameBlock = givenSudokuMap.StartRow == currentSudokuMap.StartRow &&
                        givenSudokuMap.StartCol == currentSudokuMap.StartCol;

                    if (!elementSame &&
                        elementInSameBlock &&
                        IsNakedPair(sudokuBoard[givenRow, givenCol], sudokuBoard[row, col]))
                        return true;
                }
            }
        ...
        private void EliminateNakePair(int[,] sudokuBoard, int valuesToEliminate, int eliminateFromRow, int eliminateFromCol)
        {
            ...
            foreach (var valueToEliminate in valuesToEliminateArray)
            {
                sudokuBoard[eliminateFromRow, eliminateFromCol] =
                    Convert.ToInt32(sudokuBoard[eliminateFromRow, eliminateFromCol].ToString()
                    .Replace(valueToEliminate.ToString(), string.Empty));
            }
        }

        private bool IsNakedPair(int firstPair, int secondPair) => firstPair.ToString().Length == 2 && firstPair == secondPair;
    }
}